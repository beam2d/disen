...
def ri_zi_zmi_yj(
    ...
    _logger.info("start training density ratio estimator for PUI...")
    m = model.spec.size
    B_each = 32
    B = B_each * (m + 1)
    n_epochs = 10
    ...
    def prepare_perm(zs: Sequence[torch.Tensor]) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    _logger.info("start evaluating PUI...")
    L0 = 10_000
    K = 32
    B0 = 16
    m = model.spec.size
